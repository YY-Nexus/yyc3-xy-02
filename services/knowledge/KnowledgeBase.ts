...
export class KnowledgeBase extends EventEmitter {
  ...
  async searchKnowledge(query: KnowledgeQuery): Promise<KnowledgeSearchResult> {
    try {
      ...
      const results = similarItems
        .map((item: { id: string; similarity: number }) => this.knowledgeItems.get(item.id))
  ...
  async rebuildIndex(): Promise<void> {
    ...
    try {
      ...
      // 重新添加所有项目
      Array.from(this.knowledgeItems).forEach(([id, item]) => {
        if (item.embedding) {
          this.knowledgeIndex.add(id, item.embedding)
        }
      })
  ...
  private calculateMemoryUsage(): number {
    // 简化的内存使用计算
    let totalSize = 0
    Array.from(this.knowledgeItems.values()).forEach(item => {
...
class InMemoryVectorStorage implements VectorStorage {
  ...
  async search(
    ...
    Array.from(this.vectors).forEach(([id, vector]) => {
